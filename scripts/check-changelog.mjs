...
if (!section) {
  console.error(
    "\x1b[31m" +
      `❌ Error: CHANGELOG.md must contain a ## ${version} section.` +
      "\x1b[0m",
  );
  process.exit(1);
}
...
if (!description) {
  console.error(
    "\x1b[31m" +
      `❌ Error: CHANGELOG.md must describe the changes for ${version}.` +
      "\x1b[0m",
  );
  process.exit(1);
}
...
console.log(
  "\x1b[32m" +
    `✅ Success: CHANGELOG.md contains release notes for ${version}.` +
    "\x1b[0m",
);
